...
- [ ] `nas-local`로 `.git` / `.env` / 컨테이너 상태 확인·복구
- [ ] 변경 분류(data / code / image) + `--full-build` 기본 제거
- [ ] harden: 시크릿 **신규 생성 시에만** recreate
- [ ] (선택) `app`·`server.py` 바인드 마운트로 코드 반영 = restart only
- [ ] main 동기화 후 health + 리포트 광고 차트 스모크
- [ ] 커밋·push (시크릿 제외)
